...
class Degeneracy:
    ...
    def __repr__(self) -> str:
        kind = self.degeneracy_type.value.replace("_", " ").title()
        df = f"{float(self.Delta_tilde_f):.3f}" if self.Delta_tilde_f is not None else "None"
        dk = f"{float(self.Delta_tilde_kappa):.3f}" if self.Delta_tilde_kappa is not None else "None"
        return f"{kind} @ (Δκ̃ = {dk:>7}, Δf̃ = {df:>7})"


def ep_location(phi) -> list[Degeneracy]:
    # Two EPs at each phi. Not always obvious which one is primary and secondary
    if phi == np.pi:
        return [
            Degeneracy(
...
def tpd_location(phi, kappa_tilde_c) -> list[Degeneracy]:
    ...
    degen_list = []
    if phi == 0:
        # There can either be 2, or 6 TPDs
