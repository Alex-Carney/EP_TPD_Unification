...
Changes vs previous single-panel script:
  - Panel label is now "(a)" (was "(c)")
  - Legend is above the panel (in a dedicated top band)
  - Legend is forced to 2 rows (ncol=4)
  - Fonts scaled up everywhere via FONT_SCALE
"""
...
def add_discriminant_inset(
    ...
    *,
    ...
    axins.scatter([tpd_dk], [0.0], s=200, facecolors="none", edgecolors="red", linewidths=3.0, zorder=5)
    axins.axhline(float(df_sweep), color=st["sweep_color"], linestyle=st["sweep_ls"], linewidth=st["lw_sweep"], zorder=4)
...
def build_panel_only() -> None:
    ...
    df_val = float(panel_cfg["delta_f_for_roots"])
    kappa_tilde_c = float(panel_cfg["kappa_c"])  # in your config this is already kappa-tilde_c

    # txt = (
    #     r"$\tilde{\delta}(\tilde{\Delta}_f) = $" + f"{df_val:.1e}\n"
    #     r"$\tilde{\kappa}_c = $" + f"{kappa_tilde_c:.2f}"
    # )
    #
    # ax.text(
    #     0.98,
    ...
    #     ha="right",
    #     va="top",
    #     fontsize=st["fs_panel_text"],  # or st["fs_label"] if you want slightly smaller
    # )
    ...
    # Same legend entries as before, but placed ABOVE and forced to 2 rows
    handles_theory = [
        ...
        Line2D([0, 1], [0, 1], color=st["tpd_color"], lw=4.0, ls="-",
               label=r"$\tilde{\Delta}_\kappa^{\mathrm{TPD}}$"),
